...
class BARTModel(TransformerModel):
    ...
    def forward(
        ...
        if (
            classification_head_name is not None
            and self.args.classification_aggregator == "encoder"
        ):
            x, extra = None, None
        else:
        ...
        eos: int = self.eos
        if classification_head_name is not None:
            if self.args.classification_aggregator == "decoder":
                sentence_representation = x[src_tokens.eq(eos), :].view(
                    x.size(0), -1, x.size(-1)
                )[:, -1, :]
            elif self.args.classification_aggregator == "encoder":
                sentence_representation = encoder_out.encoder_out[0, :]
            elif self.args.classification_aggregator == "encoder_decoder":
                decoder_representation = x[src_tokens.eq(eos), :].view(
                    x.size(0), -1, x.size(-1)
                )[:, -1, :]
                encoder_representation = encoder_out.encoder_out[0, :]
                sentence_representation = torch.cat(
                    [encoder_representation, decoder_representation], -1
                )
    ...
    def register_classification_head(
        ...
        self.classification_heads[name] = BARTClassificationHead(
            ...
            if self.args.classification_aggregator in ["encoder", "decoder"]
            else self.args.encoder_embed_dim + self.args.decoder_embed_dim,
            inner_dim=self.args.encoder_embed_dim
            if self.args.classification_aggregator in ["encoder", "decoder"]
            else self.args.encoder_embed_dim
            + self.args.decoder_embed_dim,  # inner_dim or self.args.encoder_embed_dim,
