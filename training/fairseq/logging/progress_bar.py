...
def progress_bar(
    ...
    if wandb_project:
        bar = WandBProgressBarWrapper(bar, wandb_project, args, run_name=wandb_run_name)
...
class WandBProgressBarWrapper(BaseProgressBar):
    ...
    def __init__(self, wrapped_bar, wandb_project, args=None, run_name=None):
        self.wrapped_bar = wrapped_bar
        if wandb is None:
        ...
        wandb.config.update(self.hydra_config_to_dict(args))
        # if args.user_dir:
    ...
    def hydra_config_to_dict(self, cfg):

        out_dict = {}
        in_dict = dict(cfg)

        def add_to_dict(in_dict, out_dict):
            for key in in_dict:
                if isinstance(in_dict[key], dict) or isinstance(
                    in_dict[key], DictConfig
                ):
                    add_to_dict(dict(in_dict[key]), out_dict)
                elif isinstance(in_dict[key], argparse.Namespace):
                    add_to_dict(vars(in_dict[key]), out_dict)
                else:
                    out_dict[key] = in_dict[key]

        add_to_dict(in_dict, out_dict)
        return out_dict
